...
LONG CSerial::Open (char* lpszDevice, DWORD dwInQueue, DWORD dwOutQueue, bool fOverlapped)
{
	// Reset error state
	m_lLastError = ERROR_SUCCESS;
	...
	// Open the device
	m_hFile = ::CreateFileA(lpszDevice,
						   GENERIC_READ|GENERIC_WRITE,
						   0,
						   0,
						   OPEN_EXISTING,
	...
	if (::GetDefaultCommConfigA(lpszDevice,&commConfig,&dwSize))
	{
...
LONG CSerial::JustWrite(const void* pData, size_t iLen, DWORD* pdwWritten, LPOVERLAPPED lpOverlapped, DWORD dwTimeout)
{
	::WriteFile(m_hFile,pData,iLen,pdwWritten,lpOverlapped);
	return 0;
}
